const { DataTypes } = require('sequelize')
const route = require('./route')
const adminUtil = require('./util')
const {
  attributeComponentMap,
  associationComponentMap,
} = require('./constants')

const defaultLogger = {
  info: msg => console.log(msg),
  error: msg => console.log(msg),
}

class Admin {
  constructor(options = {}) {
    this.data = []
    this.map = {}
    // 统一中间件
    this.middleware = options.middleware
    // 全局fields配置
    this.fields = options.fields || {}
    this.logger = options.logger || defaultLogger
    this.router = route(options.router, this)
  }

  // admin是对前端展示的配置
  register(Model, adminConfig = {}) {
    const key = Model.name
    const config = {
      Model,
      key,
      admin: this.normalizeConfig(Model, adminConfig),
    }

    this.data.push(config)
    this.map[key] = config
  }

  getAttributeComponent(attribute) {
    for (let t in attributeComponentMap) {
      const dataTypes = attributeComponentMap[t]
      for (let i = 0; i < dataTypes.length; i++) {
        if (attribute.type instanceof dataTypes[i]) {
          return t
        }
      }
    }

    return 'input'
  }

  getForeignKeyFields(Model) {
    const associations = Model.associations
    const ret = {}

    for (let i in associations) {
      const association = associations[i]
      const { associationType, foreignKey } = association

      if (associationType === 'BelongsTo') {
        ret[foreignKey] = 1
      }
    }

    return ret
  }

  getAttributes(Model, options = {}) {
    const exclude = options.exclude || []
    const attributes = Model.getAttributes()
    const foreignKeyFields = this.getForeignKeyFields(Model)
    const ret = {}

    for (let i in attributes) {
      const attribute = attributes[i]

      // 外键字段
      if (attribute.references || foreignKeyFields[i]) {
        continue
      }

      // exclude
      if (exclude.includes(attribute.fieldName)) {
        continue
      }

      const item = {
        isVirtual: attribute.type instanceof DataTypes.VIRTUAL,
        // createdAt这种自动生成的字段
        isAutoGenerated: attribute._autoGenerated,
        // name是展示时的名字，如创建时间
        // fieldName是真正提交的字段名，如createdAt
        fieldName: attribute.fieldName,
        name: attribute.fieldName,
        component: this.getAttributeComponent(attribute),
        required: !(
          attribute.allowNull || attribute.defaultValue !== undefined
        ),
      }

      if (attribute.defaultValue !== undefined) {
        item.defaultValue = attribute.defaultValue
      }

      // DataTypes.ENUM
      if (attribute.values) {
        item.options = attribute.values.map(item => {
          return {
            name: item,
            value: item,
          }
        })
      }

      ret[i] = item
    }

    return ret
  }

  /**
   * adminConfig需要序列化给前端
   */
  normalizeConfig(Model, config) {
    const { primaryKeyAttribute } = Model
    const attributes = this.getAttributes(Model, {
      exclude: config.excludeFields || [],
    })

    const singular = adminUtil.camelCase(Model.options.name.singular)
    const plural = adminUtil.camelCase(Model.options.name.plural)

    const ret = Object.assign(
      {
        name: singular,
        singular,
        plural,
        // 如何展示一个对象
        format: `{{ ${primaryKeyAttribute} }}`,

        primaryKeyAttribute,
        primaryKeyAutoIncrement:
          Model.primaryKeys[primaryKeyAttribute].autoIncrement,

        fields: {},
        listFields: Object.keys(attributes),
        filterFields: [],
        searchFields: [],
        orderFields: [],

        associations: {},
      },
      config,
    )

    ret.fields = this.normalizeFields(ret.fields, attributes)
    return ret
  }

  normalizeFields(fields, attributes) {
    const ret = {}

    for (let i in attributes) {
      const field = fields[i] || {}
      const fieldConfig = typeof field === 'string' ? { name: field } : field
      const attribute = Object.assign(
        {},
        attributes[i],
        this.fields[i] || {},
        fieldConfig,
      )

      if (typeof attribute.format === 'string') {
        attribute.format = {
          type: attribute.format,
        }
      }

      if (attribute.options && attribute.options.length) {
        attribute.component = 'select'
      }

      ret[i] = attribute
    }

    return ret
  }

  async generateAssociationOptions(config) {
    const associations = config.Model.associations
    const ret = {}

    for (let i in associations) {
      const association = associations[i]
      const target = association.target
      const adminConfig = config.admin.associations[i]
      const targetConfig = this.getConfig(target.name)

      if (!targetConfig) {
        continue
      }

      if (adminConfig && adminConfig.visible === false) {
        continue
      }

      const targetRecords = await target.findAll()
      ret[i] = targetRecords.map(item => {
        return {
          name: adminUtil.substitute(targetConfig.admin.format, item),
          value: item[target.primaryKeyAttribute],
        }
      })
    }

    return ret
  }

  setup() {
    this.data.map(config => {
      const associations = config.Model.associations
      const adminConfig = config.admin

      for (let i in associations) {
        const association = associations[i]
        const { associationType, target } = association
        const targetConfig = this.getConfig(target.name)

        if (!targetConfig) {
          continue
        }

        adminConfig.associations[i] = Object.assign(
          {
            fieldName: i,
            type: associationType,
            component: associationComponentMap[associationType],
            name: targetConfig.admin.name,
            format: targetConfig.admin.format,
            singular: targetConfig.admin.singular,
            plural: targetConfig.admin.plural,
            primaryKeyAttribute: targetConfig.admin.primaryKeyAttribute,
          },
          adminConfig.associations[i] || {},
        )
      }
    })
  }

  getConfig(key) {
    return key ? this.map[key] : this.data
  }
}

module.exports = Admin
